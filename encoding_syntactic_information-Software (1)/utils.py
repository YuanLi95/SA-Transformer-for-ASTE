...
id2sentiment = {1: 'neg', 2: 'neu', 3: 'pos'}
...
def get_aspects(tags, length, ignore_index=-1):
    ...
    for j in range(length):
        for i in range(length):
            if tags[i][j] == ignore_index: continue
            elif (tags[i][j] != 0)&(tags[i][j] != ignore_index):
                if start==-1:
                    start =i
            elif (tags[i][j]==0)|(tags[i][j]==ignore_index):
                if start!=-1:
                    spans_pa [str([start,i-1])]=1
                    start = -1
        if start!=-1:
            spans_pa[str([start,length-1])] = 1
    span = [eval(i) for i in spans_pa]
    return  span
...
def find_triplet(tags, aspect_spans, opinion_spans):
    ...
    for al, ar in aspect_spans:
        for pl, pr in opinion_spans:
            ...
            sentiment = -1
            if tag_num[3] >= tag_num[2] and tag_num[3] >= tag_num[1]:
                sentiment = 3
            elif tag_num[2] >= tag_num[1] and tag_num[2] >= tag_num[3]:
                sentiment = 2
            elif tag_num[1] >= tag_num[2] and tag_num[1] >= tag_num[3]:
                sentiment = 1
            if sentiment == -1:
                print('wrong!!!!!!!!!!!!!!!!!!!!')
                input()
